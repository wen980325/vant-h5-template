...
const pathResolve = (dir: string) => resolve(__dirname, dir).replace(/\\/g, '/')
...
export default ({ command, mode }) => {
  ...
  const getProxy = () => {
    let obj: any = {},
      urlArr: any = (env.VITE_APP_API_BASE_URL || '').split(','),
      urlDevArr: any = (env.VITE_APP_API_BASE_URL_DEV || '').split(',')
    urlArr.map((v: any, k: number) => {
      obj[v] = {
        target: urlDevArr[k], // 后台服务地址
        changeOrigin: true, // 是否允许不同源
        ws: true,
        rewrite: (path: string) => path.replace(v, ''),
      }
    })
    return obj
  }
    ...
    plugins: [
      ...
      Components({
        resolvers: [
          ...
          AntDesignVueResolver({ importStyle: false, importCss: false, importLess: false }),
        ],
